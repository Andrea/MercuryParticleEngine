...
namespace ProjectMercury.Renderers
{
    ...
    abstract public class Renderer
    {
        ...
        internal void Draw(float totalTime, Emitter emitter)
        {
            this.Prepare(totalTime, emitter);

            for (int i = 0; i < emitter.ActiveParticlesCount; i++)
            {
                Particle particle = emitter.Get(i);

                this.RenderParticle(totalTime, ref particle);
            }
        ...
        protected virtual void RenderParticle(float totalTime, ref Particle particle) { }
